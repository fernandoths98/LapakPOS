...
const outletSchema = z.object({ name: z.string().trim().min(2).max(100), code: z.string().trim().min(2).max(12).regex(/^[a-zA-Z0-9_-]+$/), address: z.string().max(240).optional(), phone: z.string().max(24).optional(), type: z.enum(["owned", "franchise"]).optional(), timezone: z.enum(OUTLET_TIMEZONES).optional() });
