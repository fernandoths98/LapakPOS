...
import { useBillers, usePpobCommissionSummary, usePpobTransactions, useWalletSummary } from "../../state/api/ppob";
...
export function BillsScreen() {
  ...
  return (
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}
      ...
      <View style={[styles.sectionHeading, styles.historyHeading]}><Text variant="h3">Transaksi terakhir</Text>{(transactionsQuery.data ?? []).length > 0 ? <Text variant="caption" color={colors.neutral600}>{(transactionsQuery.data ?? []).length} transaksi</Text> : null}</View>
      ...
      <View style={styles.historyCard}>
        ...
        {(transactionsQuery.data ?? []).map((tx, index) => <RecentRow key={tx.id} transaction={tx} last={index === (transactionsQuery.data?.length ?? 0) - 1} onPress={() => navigation.navigate("BillSuccess", { transaction: tx, fromHistory: true })} />)}
      </View>
    </ScrollView>
    </SafeAreaView>
  );
...
function RecentRow({ transaction, last, onPress }: { transaction: PpobTransaction; last: boolean; onPress: () => void }) {
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: "row", alignItems: "center", gap: space[3], padding: space[3], borderBottomWidth: 1, borderBottomColor: colors.divider }, lastRow: { borderBottomWidth: 0 }, rowPressed: { backgroundColor: colors.neutral100 }, statusIcon: { width: 34, height: 34, borderRadius: 17, alignItems: "center", justifyContent: "center" }, statusSuccess: { backgroundColor: "#EAF8F0" }, statusPending: { backgroundColor: "#FFF6DF" }, statusFailed: { backgroundColor: colors.accent100 }, rowBody: { flex: 1, minWidth: 0 }, rowTitle: { fontWeight: "600" }, rowEnd: { alignItems: "flex-end", flexShrink: 0 },
});
