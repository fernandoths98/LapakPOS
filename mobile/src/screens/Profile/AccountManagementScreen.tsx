...
import { CreateStaffRequest, isUnlimited, UserRole } from "@lapak/shared";
...
import { apiErrorMessage, isPlanLimitError } from "../../state/api/apiClient";
...
export function AccountManagementScreen({ navigation }: Props) {
  ...
  const [outletType, setOutletType] = useState<"owned" | "franchise">("owned");
  ...
  const resetForm = () => { setForm(null); setName(""); setCode(""); setPhone(""); setAddress(""); setOutletType("owned"); setEmail(""); setPin(""); setRole("cashier"); setError(null); };
  ...
  const submitOutlet = async () => {
    setError(null);
    try {
      await createOutlet.mutateAsync({ name: name.trim(), code: code.trim(), phone: phone.trim() || undefined, address: address.trim() || undefined, type: outletType });
      resetForm();
    } catch (err) {
      setError(isPlanLimitError(err) ? apiErrorMessage(err, "Batas outlet paket ini tercapai.") : "Outlet gagal dibuat. Pastikan kode belum digunakan.");
    }
  };
  const submitStaff = async () => {
    setError(null);
    try {
      const body: CreateStaffRequest = { name: name.trim(), email: email.trim() || undefined, role, outletId, pin };
      await createStaff.mutateAsync(body);
      resetForm();
    } catch (err) {
      setError(isPlanLimitError(err) ? apiErrorMessage(err, "Batas staf paket ini tercapai.") : "Staf gagal dibuat. Periksa email, outlet, dan PIN.");
    }
  };
  ...
  const outletCapReached = !!ent && !isUnlimited(ent.entitlements.maxOutlets) && ent.usage.outlets >= ent.entitlements.maxOutlets;
  const staffCapReached = !!ent && !isUnlimited(ent.entitlements.maxStaff) && ent.usage.staff >= ent.entitlements.maxStaff;
  const planName = (data.subscription?.planCode ?? "free").toUpperCase();
  const planStatus =
    data.subscription?.status === "active"
      ? data.subscription.currentPeriodEndsAt
        ? `Aktif s/d ${new Date(data.subscription.currentPeriodEndsAt).toLocaleDateString("id-ID")}`
        : "Aktif"
    ...
    <SafeAreaView style={styles.safe} edges={[]}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.accountCard}>
          ...
          <Pressable style={styles.planRow} onPress={() => navigation.navigate("Subscription")}>
            <View>
              <Text variant="body">Paket {planName}</Text>
              <Text variant="caption" color={colors.neutral600}>{planStatus}</Text>
            </View>
            <View style={styles.planRowRight}>
              <Text variant="caption" color={colors.accent2}>Kelola</Text>
        ...
        {outletCapReached ? (
          <Text variant="caption" color={colors.neutral600} style={styles.capNote}>
            Paket {planName} dibatasi {ent?.entitlements.maxOutlets} outlet. Upgrade untuk menambah cabang.
          </Text>
        ) : null}
        {form === "outlet" && !outletCapReached ? <View style={styles.formCard}><Text variant="h3">Outlet baru</Text><View style={styles.fields}><TextField label="Nama outlet" value={name} onChangeText={setName} placeholder="Cabang Sudirman" /><TextField label="Kode outlet" value={code} onChangeText={(v) => setCode(v.toUpperCase().replace(/[^A-Z0-9_-]/g, "").slice(0, 12))} placeholder="SUDIRMAN" autoCapitalize="characters" /><TextField label="Telepon" value={phone} onChangeText={setPhone} placeholder="Opsional" keyboardType="phone-pad" /><TextField label="Alamat" value={address} onChangeText={setAddress} placeholder="Opsional" /></View><Text variant="kicker" style={styles.inputLabel}>JENIS OUTLET</Text><View style={styles.choiceRow}><Choice label="Milik sendiri" active={outletType === "owned"} onPress={() => setOutletType("owned")} /><Choice label="Franchise" active={outletType === "franchise"} onPress={() => setOutletType("franchise")} /></View>{error ? <ErrorText text={error} /> : null}<View style={styles.formActions}><Button title="Batal" variant="secondary" onPress={resetForm} style={styles.flexButton} /><Button title="Simpan outlet" onPress={submitOutlet} disabled={name.trim().length < 2 || code.length < 2} loading={createOutlet.isPending} style={styles.flexButton} /></View></View> : null}
